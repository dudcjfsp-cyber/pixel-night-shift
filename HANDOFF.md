...
## 18. 2026-07-22 다음 전투 마일스톤 확정

- Android 패키징보다 게임 완성도 향상을 먼저 진행한다.
- 현재 본편의 20스테이지·일반전 가독성·유지보수 파밍을 보존하고 Combat V2 전체를 그대로 대체하지 않는다.
- 보스전에서만 요원 역할, HP·PROCESS DOWN, QA 자동 구조를 활성화한다.
- 일반전 요원 피해, 고정 6초 대기, 수동 긴급 재배포는 본편에 채택하지 않는다.
- 첫 회차 목표를 25~35분으로 두되 시간은 무행동 대기가 아니라 강화·패치·진단 판단으로 늘린다.
- 권위 구현 명세는 `docs/COMBAT_HYBRID_SPEC.md`다. 다음 작업은 이 문서의 구현 순서 1번인 20스테이지 비교 리포트 확장이다.
