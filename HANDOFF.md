...
- 진단 카드는 실패 근거와 예상 처치 시간·전원 DOWN 위험을 보여줍니다.
- 최대 2명의 요원 어필은 실제 스냅숏 근거만 사용합니다.
- 실패해도 진행을 삭제하지 않고 유지보수 파밍과 자동 재도전으로 복구합니다.
- 스테이지 20 이후 버전 업데이트로 패치노트와 영구 보너스를 남기고 다음 회차를 시작합니다.

권위 전투 명세는 `docs/COMBAT_HYBRID_SPEC.md`입니다. 구현·자동 검증 순서 1~7은 완료됐습니다. 다음 완료 관문은 내부 한 회차 확인과 5인 플레이테스트입니다.
...
- 현재 일반·보스 BGM과 효과음은 CC0 최종 에셋이며 유지보수 BGM은 프로젝트 자체 절차 합성입니다.
...
2026-07-22 혼합 전투 구현 완료 시 다음 항목이 종료 코드 0으로 통과했습니다.

- 픽셀·오디오 validator
- 핵심 도메인, 앱 셸과 AppRoot 통합 테스트
- 저장 schema 1→2 마이그레이션과 오프라인 중복 방지
- 혼합 보스 내구도·QA 구조·진단·어필 테스트
- 정책별 20스테이지 비교와 목표 밸런스
- 일반 headless 메인 장면 실행
- `git diff --check`

자동 검증은 실제 스피커 출력, 글꼴 체감과 장시간 GUI 조작을 대신하지 않습니다.

## 6. 현재 남은 작업

우선순위는 다음과 같습니다.

1. 360×640 기본 본편을 실제로 한 회차 플레이해 보스 경고, DOWN, QA 구조, 진단·어필, 패치 이동, 버전 업데이트와 오디오를 확인합니다.
2. 프로젝트 고유 타격음과 핵심 효과음 첫 묶음을 만들어 현재 CC0 버전과 A/B 비교합니다.
3. 발견된 차단 문제만 수정하고 `docs/PLAYTEST.md` 기준 5인 플레이테스트를 진행합니다.
4. 기준 통과 후 Combat V2 전용 코드와 테스트 분기를 정리합니다.

사용자 결정에 따라 Android 빌드와 실기기 검수는 현재 보류합니다. Android SDK/JDK, export template, export preset과 서명 설정은 아직 없습니다.
...
런타임 최종 파일만 `game/assets/audio/`에 둡니다. 후보, DAW 프로젝트와 중간 렌더는 Git 밖의 `D:\자체제작게임\PixelNightShift-Archive`에 보관합니다. 채택한 파일은 `game/assets/audio/manifest.json`과 `ATTRIBUTION.md`에 생성 방식·라이선스·해시를 기록합니다.
